...
export interface RedisThrottleQueue {
    /**
     * If the task hasn't been queued (or has been modified), this method adds
     * it to the queue (or update it in the queue ) and returns the signature of
     * the task, otherwise, it only returns the signature and keeps the task
     * untouched.
    ...
    /**
     * This method is like `add()`, only it returns `true` indicates the task 
     * has been successfully added or updated, and returns `false` indicates a
     * failure or the task wasn't modified.
     */
    ...
    /**
     * Removes a task from the queue by the given signature of the task
     * generated by `add()`.
     */
    delete(sign: string): Promise<boolean>;
